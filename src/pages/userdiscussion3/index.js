...
export default function UserDiscussion1() {
  ...
  const replyToQuestion = (e) => {
    ...
    Firestore.collection('questionAnswers').add({
      questionsId: questionDetails.id,
      answer: answer,
  ...
  useEffect(() => {
    ...
    Firestore.collection('questionAnswers').where('questionsId', '==', questionDetails.id)
      .get().then((allanswers) => {
  ...
  return (
    <div>
      ...
      <UserDiscussionStructure heading={questionDetails?.title}>
